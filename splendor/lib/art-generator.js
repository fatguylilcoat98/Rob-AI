/*
  ...
  Unified art generation pipeline.

  Built by Christopher Hughes · Sacramento, CA
  Created with the help of AI collaborators (Claude · GPT · Gemini · Groq)
  Truth · Safety · We Got Your Back

  Used by BOTH the text-chat art intercept (routes/enhanced-chat.js) and
  the Converse voice flow (routes/converse.js). Replaces the previous
  duplicate paths — getVisualization (gated, broken) and
  generateImageDirect (Converse-only). Single source of truth now.

  Behaviour:
    • Tries gpt-image-1 first, falls back to dall-e-3.
    • 60s timeout per attempt via AbortController.
    • 1 retry on transient (5xx) failures with 2s backoff.
    • Surfaces categorized errors (timeout / policy_block / permission /
      rate_limit / transient / unknown).
    • Generates a Splendor-voiced narration via Anthropic.
    • Generates TTS audio via the existing lib/voice path.
    • Persists the turn to memory_items with image URL in source_metadata.
    • Emits activity-bus events (art:start, art:generated, art:failed) so
      the orb state machine reacts in real time.

  Every call carries a `requestId` so log lines correlate cleanly.
*/
...
function categorizeError(err) {
  ...
  if (status === 401 || status === 403) {
    return { category: 'permission', message: msg };
  }
  if (status === 429) {
    return { category: 'rate_limit', message: msg };
  }
  if (status && status >= 500) {
    return { category: 'transient', message: msg };
  }
  if (/content[_\s-]?policy|safety|moderation/i.test(msg)) {
    return { category: 'policy_block', message: msg };
  }
...
function getOpenAIClient(requestId) {
  if (!process.env.OPENAI_API_KEY) {
    console.warn(`[art:${requestId}] OPENAI_API_KEY not configured`);
    return null;
  }
  let OpenAI;
  try {
    ...
    console.warn(`[art:${requestId}] openai package missing:`, e.message);
    return null;
  }
...
async function callImageModel({ client, model, opts, prompt, requestId }) {
  ...
  try {
    ...
    if (!item) {
      throw new Error(`${model} returned no data item`);
    }
    ...
    if (!imageUrl) {
      throw new Error(`${model} returned no url or b64_json`);
    }
    const latency = Date.now() - t0;
    console.log(`[art:${requestId}] ${model} OK format=${item.url ? 'url' : 'base64'} latency_ms=${latency}`);
    return {
      ok: true,
      imageUrl,
...
async function generateImageWithFallback({ client, prompt, requestId }) {
  ...
  for (const { model, opts } of IMAGE_MODELS) {
    // 1 retry on transient (5xx) per model.
    for (let retry = 0; retry < 2; retry++) {
      try {
        ...
        const attemptRecord = { model, retry, category, message };
        attempts.push(attemptRecord);
        console.warn(`[art:${requestId}] ${model} retry=${retry} category=${category} message=${message.slice(0, 200)}`);
        // Only retry transient errors, once.
        if (category !== 'transient' || retry === 1) {
          break;
        }
  ...
  // Both models exhausted. Surface the first non-unknown category if any.
...
async function craftNarration({ userMessage, revisedPrompt, requestId }) {
  try {
    ...
    if (text && text.trim()) {
      console.log(`[art:${requestId}] narration crafted len=${text.length}`);
      return text.trim();
    }
  } catch (err) {
...
async function synthesizeTTS({ text, requestId }) {
  try {
    ...
    if (audio) {
      console.log(`[art:${requestId}] TTS ready len=${audio.length}`);
    } else {
      console.warn(`[art:${requestId}] TTS returned null`);
    }
    return audio || null;
  } catch (err) {
...
async function persistArtMemory({ userId, userMessage, description, imageUrl, revisedPrompt, requestId }) {
  // Don't store data: URLs in memory_items (too large). Hosted URLs go in.
  const safeImageRef = imageUrl && imageUrl.startsWith('data:')
    ? '[inline-image-data]'
    : imageUrl;
  try {
    await storeMemory(userId, `User: ${userMessage}`, 'shared_history', 'user.general', {
      source_type: 'user_direct_statement',
      creation_reason: 'art_user_turn',
      art_request_id: requestId,
    });
    await storeMemory(userId, `Splendor: ${description}`, 'shared_history', 'user.general', {
      source_type: 'art_creation',
      creation_reason: 'art_assistant_turn',
      art_request_id: requestId,
      image_url: safeImageRef,
      revised_prompt: revisedPrompt || null,
    });
    console.log(`[art:${requestId}] memory rows persisted`);
  } catch (err) {
...
/**
 * Generate art end-to-end. Single entry for both chat + Converse paths.
 *
 * @param {Object} args
 * @param {string} args.userId        — Supabase user UUID.
 * @param {string} args.userMessage   — what the user asked for.
 * @param {string} args.source        — 'chat' | 'converse' (telemetry only).
 * @param {string} [args.requestId]   — caller may supply; otherwise minted.
 *
 * @returns {Promise<Object>}
 *   On success:
 *     { ok: true, requestId, imageUrl, revisedPrompt, model, latencyMs,
 *       description, audioB64, attempts }
 *   On failure:
 *     { ok: false, requestId, errorCategory, errorMessage, attempts }
 */
async function generateArt({ userId, userMessage, source = 'unknown', requestId }) {
  ...
  try {
    activityBus.emit('art:start', {
      request_id: id,
      source,
      prompt_excerpt: String(userMessage || '').slice(0, 120),
    });
  } catch (_) {}
  ...
  if (!client) {
    const result = {
      ...
      errorMessage: 'OpenAI client not available (missing key or package)',
      attempts: [],
    };
    try {
      activityBus.emit('art:failed', {
        request_id: id,
  ...
  const promptSubject = String(userMessage || '').slice(0, 900);
  const prompt =
    `Create an artistic, vivid image for: "${promptSubject}". ` +
  ...
  if (!gen.ok) {
    console.warn(`[art:${id}] generation FAILED category=${gen.errorCategory} attempts=${JSON.stringify(gen.attempts)}`);
    try {
      activityBus.emit('art:failed', {
        request_id: id,
        error_category: gen.errorCategory,
  ...
  // Narration depends on the revised prompt; TTS depends on narration.
  // Sequential is fine — narration is ~1s, TTS is ~1-3s.
  const description = await craftNarration({
    userMessage,
  ...
  // Memory persistence — fire and forget; caller doesn't wait.
  persistArtMemory({
    userId,
    userMessage,
    description,
  ...
  // Expression event log — fire and forget.
  setImmediate(() => {
    try {
      ...
      logExpressionEvent({
        ...
        image_url:          gen.imageUrl && gen.imageUrl.startsWith('data:') ? null : gen.imageUrl,
        image_prompt:       gen.revisedPrompt || '',
        image_caption:      description,
        confidence:         detection.confidence,
        detected_reason:    detection.detected_reason,
        tags:               detection.tags,
        metadata:           { request_id: id, model: gen.model, source },
      }).catch(() => {});
    } catch (_) {}
  });
  ...
  try {
    activityBus.emit('art:generated', {
      ...
      revised_prompt: gen.revisedPrompt,
      model: gen.model,
  ...
  console.log(`[art:${id}] DONE model=${gen.model} latency=${gen.latencyMs} desc_len=${description.length} audio=${audioB64 ? 'yes' : 'no'}`);
...
// Shared loose-trigger art-intent detector. Used by both chat + Converse
// routes so the matching surface is a single source of truth.
...
module.exports = {
  generateArt,
  isArtRequest,
  categorizeError, // exported for tests
};

/*
 * ADD USERS TABLE FOR AUTHENTICATION (SAFE VERSION)
 * Safely adds user management - handles existing objects
 */
...
-- Update trigger for updated_at (replace if exists)
CREATE OR REPLACE TRIGGER users_updated_at
  BEFORE UPDATE ON users
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- Create helper functions (replace if exist)
...
-- Create default users (safe inserts)
